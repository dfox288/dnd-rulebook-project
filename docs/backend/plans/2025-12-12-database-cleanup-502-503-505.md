...
### Task 2.5: Create Migration to Populate creature_type_id

**Files:**
- Create: Migration to populate creature_type_id from type string

**Step 1: Create data migration**

```bash
docker compose exec php php artisan make:migration populate_monster_creature_type_ids
```
...
{
    /**
     * Maps the base creature type from monster.type string to creature_type slug.
     * Example: "humanoid (elf)" -> "humanoid", "fiend (demon)" -> "fiend"
     */
    public function up(): void
    {
        // Get all creature types keyed by slug
        $creatureTypes = CreatureType::pluck('id', 'slug');

        Monster::query()
            ->whereNull('creature_type_id')
            ->chunkById(100, function ($monsters) use ($creatureTypes) {
                foreach ($monsters as $monster) {
                    $baseType = $this->extractBaseType($monster->type);
                    $slug = strtolower($baseType);

                    if (isset($creatureTypes[$slug])) {
                        $monster->update(['creature_type_id' => $creatureTypes[$slug]]);
                    }
                }
            });
    }
    ...
    public function down(): void
    {
        Monster::query()->update(['creature_type_id' => null]);
    }
...
```bash
docker compose exec php php artisan tinker --execute="
\$total = \App\Models\Monster::count();
\$populated = \App\Models\Monster::whereNotNull('creature_type_id')->count();
\$unpopulated = \App\Models\Monster::whereNull('creature_type_id')->count();
echo 'Total: ' . \$total . PHP_EOL;
echo 'Populated: ' . \$populated . PHP_EOL;
echo 'Unpopulated: ' . \$unpopulated . PHP_EOL;
if (\$unpopulated > 0) {
    echo 'Unmatched types:' . PHP_EOL;
    \App\Models\Monster::whereNull('creature_type_id')->distinct()->pluck('type')->each(fn(\$t) => print('  - ' . \$t . PHP_EOL));
}
"
```

Expected: Most monsters populated, minimal unpopulated (investigate any unmatched)

**Step 5: Commit**

```bash
git add database/migrations/*populate_monster_creature_type*
git commit -m "feat(#503): populate monster creature_type_id from type string"
```
